...
WITH contrato_lst AS(
  ...
  WHERE (pp.estado = ANY (ARRAY [ 'anticipado',
   'devengado', 'pagado' ,
    'devuelto', 'contabilizado', 'aplicado' ])) AND
        pp.estado <> 'anulado'
  GROUP BY con.id_contrato,
           con.numero,
           con.objeto,
    ...
    SELECT pro.desc_proveedor,
           ...
           c.objeto,
           c.monto AS monto_contrato,
           sum(COALESCE(devengado_pagado.monto, 0)) + sum(COALESCE(
           devengado.monto, 0)) + sum(COALESCE(
           devengado_pagado_1c.monto, 0)) + sum(COALESCE(
           ant_aplicado.monto, 0)) AS monto_devengado,
           sum(COALESCE(devengado_pagado.monto_anticipo, 0)) + sum(
           COALESCE(devengado.monto_anticipo, 0)) + sum(COALESCE(
           devengado_pagado_1c.monto_anticipo, 0)) AS monto_anticipo,
           sum(COALESCE(pagado.liquido_pagable, 0)) + sum(COALESCE(
           devengado_pagado_1c.liquido_pagable, 0)) + sum(COALESCE(
           anticipo.liquido_pagable, 0)) + sum(COALESCE(
           ant_parcial.liquido_pagable, 0)) + sum(COALESCE(
           dev_garantia.liquido_pagable, 0)) AS liquido_pagable_total,
           sum(COALESCE(anticipo.liquido_pagable, 0)) + sum(COALESCE(
           ant_parcial.liquido_pagable, 0)) AS anticipo_iquido_pagable,
           sum(COALESCE(devengado_pagado.descuento_anticipo, 0)) + sum(
           COALESCE(devengado.descuento_anticipo, 0)) + sum(COALESCE(
           devengado_pagado_1c.descuento_anticipo, 0)) AS
            descuento_anticipo,
           sum(COALESCE(ant_aplicado.monto, 0)) AS anticipo_aplicado,
           sum(COALESCE(devengado_pagado.monto_retgar_mo, 0)) + sum(
           COALESCE(devengado.monto_retgar_mo, 0)) + sum(COALESCE(
           devengado_pagado_1c.monto_retgar_mo, 0)) AS monto_retgar_mo,
           sum(COALESCE(dev_garantia.liquido_pagable, 0)) AS
            dev_garantia_iquido_pagable
    FROM leg.tcontrato c
         JOIN param.vproveedor pro ON pro.id_proveedor = c.id_proveedor
         LEFT JOIN contrato_lst devengado_pagado ON devengado_pagado.id_contrato
          = c.id_contrato AND devengado_pagado.tipo = 'devengado_pagado'
           
         LEFT JOIN contrato_lst devengado ON devengado.id_contrato =
          c.id_contrato AND devengado.tipo = 'devengado' 
         LEFT JOIN contrato_lst devengado_pagado_1c ON
          devengado_pagado_1c.id_contrato = c.id_contrato AND
           devengado_pagado_1c.tipo = 'devengado_pagado_1c' 
         LEFT JOIN contrato_lst anticipo ON anticipo.id_contrato = c.id_contrato
          AND anticipo.tipo = 'anticipo' 
         LEFT JOIN contrato_lst ant_parcial ON ant_parcial.id_contrato =
          c.id_contrato AND ant_parcial.tipo = 'ant_parcial' 
         LEFT JOIN contrato_lst dev_garantia ON dev_garantia.id_contrato =
          c.id_contrato AND dev_garantia.tipo = 'dev_garantia' 
         LEFT JOIN contrato_lst ant_aplicado ON ant_aplicado.id_contrato =
          c.id_contrato AND ant_aplicado.tipo = 'ant_aplicado' 
         LEFT JOIN contrato_lst pagado ON pagado.id_contrato = c.id_contrato AND
          pagado.tipo = 'pagado' 
    GROUP BY pro.desc_proveedor,
             pro.id_proveedor,
...
AS
  SELECT lban.id_libro_bancos,
         ...
         lban.estado,
         ((lban.id_int_comprobante || ' - ' ) ||
          lban.nro_comprobante)  AS nro_comprobante,
         lban.indice,
         lban.estado_reg,
         lban.tipo,
         lban.nro_deposito,
         ...
         fin.color,
         CASE
           WHEN lban.tipo = 'deposito'  THEN lban.importe_deposito -
            COALESCE((
                       SELECT COALESCE(sum(lb.importe_cheque), 0) AS
                        "coalesce"
                       FROM tes.tts_libro_bancos lb
                       WHERE lb.id_libro_bancos_fk = lban.id_libro_bancos AND
                             (lb.tipo <> ALL (ARRAY [ 'deposito'
                              , 'transf_interna_haber'
                                ]))
         ), 0) + COALESCE((
           ...
           WHEN (lban.tipo = ANY (ARRAY [ 'cheque' , 'debito_automatico' ,
             'transferencia_carta' ,
              'transf_interna_debe'  ])) AND
               lban.id_libro_bancos_fk IS NOT NULL THEN ((
                                                           SELECT COALESCE(
                                                           lb.importe_deposito,
                                                            0) AS
                ...
                SELECT COALESCE(sum(lb.importe_deposito), 0) AS
                 "coalesce"
                FROM tes.tts_libro_bancos lb
                WHERE lb.id_libro_bancos_fk = lban.id_libro_bancos_fk AND
                      (lb.tipo = ANY (ARRAY [ 'deposito' , 'transf_interna_haber'  ]))
         )) -((
                SELECT sum(lb2.importe_cheque) AS sum
                FROM tes.tts_libro_bancos lb2
                WHERE lb2.id_libro_bancos <= lban.id_libro_bancos AND
                      lb2.id_libro_bancos_fk = lban.id_libro_bancos_fk AND
                      (lb2.tipo <> ALL (ARRAY [ 'deposito' , 'transf_interna_haber'  ]))
         ))
           ELSE 0
         END AS saldo_deposito,
         reg.nombre_regional,
  ...
  FROM tes.tts_libro_bancos lban
       ...
       LEFT JOIN segu.tusuario usu2 ON usu2.id_usuario = lban.id_usuario_mod
       JOIN tes.tfinalidad fin ON fin.id_finalidad = lban.id_finalidad
       LEFT JOIN param.tregional reg ON reg.codigo_regional =
        lban.origen
  ORDER BY lban.fecha DESC;
...
AS
  ...
  FROM tes.tobligacion_pago op
       JOIN tes.tobligacion_det od ON od.id_obligacion_pago =
        op.id_obligacion_pago AND od.estado_reg = 'activo' 
       JOIN param.vproveedor pr ON pr.id_proveedor = op.id_proveedor
       JOIN param.tmoneda mo ON mo.id_moneda = op.id_moneda
       JOIN param.tgestion ges ON ges.id_gestion = op.id_gestion
       LEFT JOIN leg.tcontrato con ON con.id_contrato = op.id_contrato
  WHERE op.tipo_obligacion <> ALL (ARRAY [ 'adquisiciones' , 'rrhh'  ])
  GROUP BY op.fecha,
           op.num_tramite,
           pr.desc_proveedor,
           mo.codigo,
           ...
           op.ultimo_estado_pp,
           ges.gestion
  HAVING sum(od.monto_pago_mb) >= 20000
  ORDER BY op.fecha DESC;
...
AS
  SELECT ppp.id_plan_pago,
         ...
         con.id_contrato,
         (('(id:'  || con.id_contrato) || ') Nro: ' ) ||
          con.numero AS desc_contrato,
         fun.desc_funcionario1
  FROM tes.tobligacion_pago op
       JOIN param.tgestion ges ON ges.id_gestion = op.id_gestion
       JOIN param.vproveedor pro ON pro.id_proveedor = op.id_proveedor
...
AS
  SELECT ppp.id_plan_pago,
         ...
         con.id_contrato,
         (((('(id:'  || con.id_contrato) || ') Nro: ' ) ||
          con.numero) || ' BAN: ' ) || con.bancarizacion AS
           desc_contrato,
         fun.desc_funcionario1,
         con.bancarizacion,
         ppp.id_proceso_wf,
...
CREATE OR REPLACE VIEW tes.vplan_pago(
    id_plan_pago,
    id_gestion,
    gestion,
    id_obligacion_pago,
...
AS
  ...
  FROM tes.tproceso_caja pc
       JOIN tes.tcaja ca ON pc.id_caja = ca.id_caja
       JOIN tes.tcajero c ON c.id_caja = ca.id_caja AND c.tipo =
        'responsable' 
       JOIN orga.vfuncionario f ON f.id_funcionario = c.id_funcionario
  WHERE pc.fecha >= c.fecha_inicio AND
        pc.fecha <= c.fecha_fin OR
        pc.fecha >= c.fecha_inicio AND
        c.fecha_fin IS NULL;
...
AS
  SELECT srd.id_solicitud_rendicion_det,
         ...
         dcv.importe_doc,
         dcv.importe_excento,
         COALESCE(dcv.importe_excento, 0) + COALESCE(dcv.importe_ice,
          0) AS importe_total_excento,
         dcv.importe_descuento,
         dcv.importe_descuento_ley,
         dcv.importe_ice,
         dcv.importe_it,
         ...
         dcv.id_doc_compra_venta,
         (((dcv.razon_social || ' - ' ) || ' ( ' ) ||
          ' ) Nro Doc: ' ) || COALESCE(dcv.nro_documento)  AS
           descripcion,
         dcv.importe_neto,
         dcv.importe_anticipo,
         dcv.importe_pendiente,
...
AS
  SELECT srd.id_solicitud_rendicion_det,
         ...
         dco.id_doc_concepto,
         cig.desc_ingas,
         (((((((dcv.razon_social || ' - ' ) || cig.desc_ingas)
          || ' ( ' ) || dco.descripcion) || ' ) Nro Doc: ' ) ||
           COALESCE(dcv.nro_documento) ) || ' Nt: ' ) || COALESCE(
           se.nro_tramite, '' )  AS descripcion,
         dcv.importe_neto,
         dcv.importe_anticipo,
         dcv.importe_pendiente,
         dcv.importe_retgar,
         dco.precio_total_final,
         (COALESCE(dcv.importe_excento, 0) + COALESCE(dcv.importe_ice,
          0)) / dcv.importe_neto AS porc_monto_excento_var,
         se.nro_tramite AS nro_tramite_se
  FROM tes.tsolicitud_rendicion_det srd
...
AS
  SELECT op.id_usuario_reg,
   ...
   <TH>Concepto</TH>
   <TH>Detalle</TH>
   <TH>Importe ('  || mon.codigo) || ')</TH>' ) ||
    pxp.html_rows(((((((((('<td>'  || ci.desc_ingas) || '</td>
       <td>' ) || '<font hdden=true>' ) ||
        od.id_obligacion_det) || '</font>' ) ||
         od.descripcion) || '</td> <td>' ) || od.monto_pago_mo) ||
          '</td>' ) ) ) || '</table>'  AS
           detalle
  FROM tes.tobligacion_pago op
       JOIN param.tmoneda mon ON mon.id_moneda = op.id_moneda
       ...
       JOIN param.tconcepto_ingas ci ON ci.id_concepto_ingas =
        od.id_concepto_ingas
       JOIN orga.vfuncionario_cargo fun ON fun.id_funcionario =
        op.id_funcionario AND fun.estado_reg_asi = 'activo' 
       JOIN segu.vusuario usu ON usu.id_usuario = op.id_usuario_reg
  GROUP BY op.id_usuario_reg,
           op.id_usuario_mod,
           op.fecha_reg,
...
AS
  ...
  FROM tes.tplan_pago pp
       ...
       LEFT JOIN orga.vfuncionario_persona fun ON op.id_funcionario =
        fun.id_funcionario
  WHERE (pp.estado::text = ANY (ARRAY [ 'borrador' ::text, 'vbsolicitante'
   ::text ])) AND
        pp.fecha_tentativa <= now() AND
        pp.monto_ejecutar_total_mo > 0::numeric AND
        (pp.tipo::text = ANY (ARRAY [ 'devengado_pagado' ::text,
         'devengado_pagado_1c' ::text, 'ant_aplicado' ::text ])) AND
        (op.tipo_obligacion::text = ANY (ARRAY [ 'adquisiciones' ::text,
         'pago_directo' ::text ])) AND
        pp.estado_reg::text = 'activo' ::text
  GROUP BY pp.id_plan_pago,
           pp.fecha_tentativa,
           pp.id_estado_wf,
